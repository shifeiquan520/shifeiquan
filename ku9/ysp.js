...
function main(item) {
    ...
    // Fallback: call PHP backend (same directory as this JS)
    try {
        var lastSlash = urlStr.lastIndexOf('/');
        var baseDir = lastSlash >= 0 ? urlStr.substring(0, lastSlash + 1) : '';
        var qmIdx = urlStr.indexOf('?');
        var qStr = qmIdx >= 0 ? urlStr.substring(qmIdx) : '';
